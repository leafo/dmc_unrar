...
- Phase 4 subset: RAR5 solid extraction now tracks RAR5 block boundaries
  ...
  output offset as the packed-stream base. The `rar` 7.x `solid.rar`
  corpus archive extracts sequentially and out of order, and
  `test/diff.sh` now compares both `simple.rar` and `solid.rar` against
  system `unrar`.
