...
- Phase 4 subset: RAR5 solid extraction now tracks RAR5 block boundaries
  ...
  cached during open-time solid-chain linking instead of recomputed on
  every skipped part. The `rar` 7.x `solid.rar` corpus archive extracts
  sequentially and out of order, and `test/diff.sh` now compares both
  `simple.rar` and `solid.rar` against system `unrar`.
...
- [x] Add differential extraction tests against `unrar`. (`test/diff.sh`,
      currently covers `simple.rar` and `solid.rar`.)
...
- [x] Debug RAR5 solid extraction state reuse.
- [x] Add tests for RAR5 solid extraction in sequential order.
- [x] Add tests for RAR5 solid extraction out of order.
- [x] Add tests for RAR5 solid archives generated by current `rar`.
- [x] Cache solid packed-input offsets at open time to avoid repeated
      chain walks during extraction.
...
- [x] Generated RAR5 solid archives extract byte-identically to `unrar`.
- [x] Sequential and out-of-order solid extraction both behave predictably.
- [~] CRC validation catches corrupted output without crashing.
      (Sequential and out-of-order tests use `validate_crc=true`; the
      broader CRC-boundary audit remains open.)
...
- [x] RAR5 solid archives generated by `rar 7.12` list correctly but corrupt or
