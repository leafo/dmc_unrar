...
if [ $# -lt 1 ]; then
  ...
  echo "targets: fuzz_open_mem fuzz_filename_stat fuzz_extract_mem fuzz_extract_solid_mem fuzz_open_cb fuzz_extract_cb fuzz_cancel" >&2
  exit 2
fi
...
case "$target" in
  fuzz_open_mem|fuzz_filename_stat|fuzz_extract_mem|fuzz_extract_solid_mem|fuzz_open_cb|fuzz_extract_cb|fuzz_cancel) ;;
